...
void pulseCameraRelay() {
    rtc_gpio_init(KY002S_TRIGGER);
    rtc_gpio_set_direction(KY002S_TRIGGER, RTC_GPIO_MODE_OUTPUT_ONLY);

    rtc_gpio_set_level(KY002S_TRIGGER, HIGH);
    delay(PULSE_MS);
    rtc_gpio_set_level(KY002S_TRIGGER, LOW);    // Trigger latch
...
void setup() {
    ...
    if (cause != ESP_SLEEP_WAKEUP_EXT0) {
        ...
        initRelayState();              // Read status pin, pulse OFF if needed
        enterDeepSleepEXT0();
        return;
    }
...
void enterDeepSleepEXT0() {
    ...
    rtc_gpio_deinit(KY002S_TRIGGER);
