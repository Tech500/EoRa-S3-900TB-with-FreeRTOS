...
void initRadio(){
   SPI.begin(RADIO_SCLK_PIN, RADIO_MISO_PIN, RADIO_MOSI_PIN, RADIO_CS_PIN);
    ...
    // RX-only logic — DISABLE for TX
    
    // set the function that will be called
    // when new packet is received
    radio.setDio1Action(setFlag);

    // ONLY the Receiver should start DutyCycleAuto immediately
    // For now, let's keep the Transmitter in Standby until it's ready to TX
    radio.standby(); 
    Serial.println(F("Radio Initialized in Standby."));
