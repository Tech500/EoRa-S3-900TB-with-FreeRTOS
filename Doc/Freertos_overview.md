...
// In wifi_task:
xQueueReceive(dataQueue, &sensorReading, portMAX_DELAY); // → sends it over WiFi when ready
```
...
| Task                | Priority | Why                              |
|---------------------|----------|----------------------------------|
| Safety/alarm logic  | HIGH     | Must always respond instantly    |
| Sensor processing   | MEDIUM   | Regular, timely updates needed   |
| WiFi transmission   | LOW      | Slow, tolerant of delays         |
